...
p2214 **完全背包** 我真tmd不会写背包题目啊 咋办 完全背包 奶牛 看农场里最小多少牛  

p2629 倒叙让数组前缀和永不小于零 好题 指有很多思路去写  
断环为链的思想 很像前面 环状最大子段和 9.13 也有点像前面一道 bfs 展开走无限远的地图 9.06  
可以单调队列维护区间最小值 也可以是线段树来维护最小 st表维护要加几句 但我也留一份吧 ~~毕竟我不会维护这个~~  

p2671 纸带求和 y-x=z-y colorx=colorz 说明y根本没啥用 处理xz奇偶性相同即可  
然后推式子发现处理前缀和 ~~我不会 直接抄题解的~~  

p2721 投资 纯dp 我就想自己写出一道dp来这么难? md 搞错了 一定要先遍历时间才行 不能多次遍历时间  

p1990 砌墙 dp 没几行代码纯考虑状态转移过去  

p2804 求区间均值是否大于一个数 全部减去数 然后算前缀和 求顺序对  
我tm算不出来啊 md 啊喂 树状数组 我就是写写不对 错几个tle  

B3614 舒缓心情写模板 cin.getline(c,10) 淦 不是谁用这个?  
直接string cin>>string 不就好了? 我真傻真的  

## 9.19 5/5 day12
the 12th day  
周二 大物一早上+数据结构de三节课+中午抢物理实验+没饭买肠粉+下午马原抄实验报告+回来  
一天就这样完了? 时间都去哪里了  

4道学校数据结构题2的1-4
2-1 验证表 直接map存了对应关系 过了  
2-2 求循环小数 更sb 意思就是用每次算用链表存下一位 最后遍历链表 那直接算然后输出不就好了?  
2-3 双向约瑟夫 这个de老久 就是没处理好m=1的情况  
2-4 更离谱 就写俩接口函数 访问就好  

p3128 树上差分+lca 真的全新知识 一方面lca算法 一方面差分我不会  
点差分 通过回溯法算出访问最大值  

## 9.20 /5 day13
the 13th day  
经典满课星期三 麻了 大物 概率论 离散 数值分析 光学 就今晚还开推优大会  
什么jb安排啊 真的日了狗了  

行吧 那5道题肯定写不完了 还要补补课上内容 写一点算一点吧

p3155 树形dp 看不懂题目 只知道是转移 部分结点被上了黑或白色  
要求找到 根节点到每个叶子结点的路径上都有至少一个颜色为c[i]的点  

p3452 **将图内的点分尽可能多的集合 满足任意不在一个集合里的点之间都能有边相连**  
建补图(原图有的边无 原图无的边有) 在补图里面弄连通块 连通块在原图里没有相连的边  
把访问点就删掉 vis=1 然后类似bfs 扫他所有能到的点 cov=1  
再遍历全部存在的点一次 若访问不到就加进kuai++ 删去 能访问到就重新赋值为0  